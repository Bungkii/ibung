...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F9FAFB] dark:bg-[#0A0F0A] p-4">
      {isResetMode ? (
        // 🎯 หน้าต่างลืมรหัสผ่าน
        <div className="w-full max-w-md mx-auto p-8 bg-white dark:bg-[#1A241A] rounded-[2rem] shadow-2xl border dark:border-green-900/30">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-green-600 mb-2 flex justify-center items-center gap-2"><Sparkles size={28}/> ibung</h1>
        ...
        // 🎯 หน้าต่างเข้าสู่ระบบ (มีปุ่ม Google แน่นอน!!)
        <div className="w-full max-w-md mx-auto p-8 bg-white dark:bg-[#1A241A] rounded-[2rem] shadow-2xl border dark:border-green-900/30">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-green-600 mb-2 flex justify-center items-center gap-2"><Sparkles size={28}/> ibung</h1>
          ...
          {/* 💥 นี่ไงพี่!! ปุ่ม Google ใหญ่เต็มตา!! 💥 */}
          <div className="mb-6">
            <button onClick={handleGoogleLogin} disabled={loading} type="button" className="w-full flex items-center justify-center gap-3 bg-white dark:bg-[#0D140D] border border-gray-200 dark:border-green-900/30 text-gray-700 dark:text-gray-200 py-3.5 rounded-xl font-bold hover:bg-gray-50 dark:hover:bg-green-900/10 transition-all active:scale-95 shadow-sm">
          ...
          <div className="flex items-center my-6">
            ...
            <span className="px-4 text-xs text-gray-400 font-bold uppercase tracking-wider">หรือใช้อีเมล</span>
            <div className="flex-1 border-t border-gray-200 dark:border-green-900/30"></div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {!isLogin && (
              <div className="relative">
                ...
                <input type="text" placeholder="ชื่อแสดงผล (Display Name)" value={name} onChange={e => setName(e.target.value)} required={!isLogin} className="w-full pl-12 pr-4 py-3.5 bg-gray-50 dark:bg-[#0D140D] text-gray-900 dark:text-white rounded-xl outline-none focus:ring-2 focus:ring-green-500 transition-all" />
              </div>
            )}
            <div className="relative">
              <Mail className="absolute left-4 top-3.5 text-gray-400 w-5 h-5" />
              <input type="email" placeholder="อีเมล" value={email} onChange={e => setEmail(e.target.value)} required className="w-full pl-12 pr-4 py-3.5 bg-gray-50 dark:bg-[#0D140D] text-gray-900 dark:text-white rounded-xl outline-none focus:ring-2 focus:ring-green-500 transition-all" />
            </div>
            
            <div>
              <div className="relative">
                <Lock className="absolute left-4 top-3.5 text-gray-400 w-5 h-5" />
                <input type="password" placeholder="รหัสผ่าน" value={password} onChange={e => setPassword(e.target.value)} required className="w-full pl-12 pr-4 py-3.5 bg-gray-50 dark:bg-[#0D140D] text-gray-900 dark:text-white rounded-xl outline-none focus:ring-2 focus:ring-green-500 transition-all" />
              </div>
              {isLogin && (
            ...
            <button type="submit" disabled={loading} className="w-full bg-green-500 text-white font-bold py-3.5 rounded-xl hover:bg-green-400 active:scale-95 transition-all flex justify-center items-center gap-2 shadow-lg shadow-green-500/30 mt-2">
              {loading && <Loader2 className="animate-spin w-5 h-5" />}
              {isLogin ? "เข้าสู่ระบบด้วยอีเมล" : "สร้างบัญชีใหม่"}
            </button>
          </form>
