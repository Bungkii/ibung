...
export default function AuthForm() {
  ...
  // 🎯 ฟังก์ชันเช็คและบันทึกข้อมูลลง Database
  const checkAndSaveUser = async (user: any) => {
    const userRef = ref(db, `users/${user.uid}`);
    const snapshot = await get(userRef);
    // ถ้าเพิ่งล็อกอินครั้งแรก ให้สร้างโปรไฟล์ใน Database ด้วย
    if (!snapshot.exists()) {
      await set(userRef, {
        uid: user.uid,
  ...
  // 🟢 ล็อกอินด้วย Google (ลบ Apple ออกแล้ว!)
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      router.push("/chat"); // ล็อกอินเสร็จ วาร์ปไปหน้าแชท
    } catch (err: any) {
  ...
  return (
    <div className="w-full max-w-md mx-auto p-8 bg-white dark:bg-[#1A241A] rounded-[2rem] shadow-2xl border dark:border-green-900/30">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {!isLogin && (
          <div className="relative">
            ...
            <input type="text" placeholder="ชื่อแสดงผล (Username)" value={name} onChange={e => setName(e.target.value)} required={!isLogin} className="w-full pl-12 pr-4 py-3 bg-gray-50 dark:bg-[#0D140D] text-gray-900 dark:text-white rounded-xl outline-none focus:ring-2 focus:ring-green-500 transition-all" />
          </div>
        )}
        <div className="relative">
      ...
      <div className="mt-6 text-center text-sm font-medium text-gray-500">
        ...
        <button onClick={() => { setIsLogin(!isLogin); setError(""); }} className="text-green-600 hover:underline font-bold">
